<?php
   ...
   // COMPROBAMOS SI EL NICK NO CONTIENE CARACTERES ESPECIALES
      if($NicKNoSan!=$Nick)
        {
             header('location:GestionDeAdmnistradores.php?ER=2');
             exit;
        }

   // COMPROBAMOS SI EL NUEVO NICK QUE INTRODUCE EL USUARIO EXITE
      $sqlNick="select Nick from tblusuario where IdUsuario!=".$_SESSION['IDUsuario']."";
      ...
      if($existe!=0)
      {
         header('location:GestionDeAdmnistradores.php?ER=1');
         exit;
      }
                                            ...
                                            CodPostal='$Cp',
                                                Telefono='$Tel',
                                                    FechaNacimiento='$FechNac',
                                                        Nick='$Nick',
                                                            Contrasenia='$Pass' where IdUsuario=".$_SESSION['IDUsuario']."";

       $DatUserMod=$conexion->query($sqlUserMod);

        if (!$DatUserMod)
         {
            echo "Error al modificar los datos del usuario. Error 400<br/>";
            exit;
         }
        else
        {
          header('location:GestionDeAdmnistradores.php?DM=1');
        }
?>
