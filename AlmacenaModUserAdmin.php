<?php
   ...
   // COMPROBAMOS SI EL NICK NO CONTIENE CARACTERES ESPECIALES
      if($NicKNoSan!=$Nick)
        {
             header('location:ModificarUsuariosAdmin.php?Id='.$Id.'&ModOP=1&ER=2');
             exit;
        }
   ...
   // COMPROBAMOS SI EL NUEVO NICK QUE INTRODUCE EL USUARIO EXITE
      ...
      if($existe!=0)
      {
         header('location:ModificarUsuariosAdmin.php?Id='.$Id.'&ModOP=1&ER=1');
         exit;
      }
   ...
   // MODIFICAMOS LA INFORMACION DEL USUARIO EN LA TABLA tblusuarios
       ...
       $DatUserMod=$conexion->query($sqlUserMod);
        ...
        {
          header('location:ModificarUsuariosAdmin.php?Id='.$Id.'&DM=1');
        }
?>
